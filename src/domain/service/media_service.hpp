...
class IMediaService {
   public:
    ...
    // 初始化分片上传
    virtual Result<model::UploadSession> InitMultipartUpload(const uint64_t user_id, const std::string &file_name,
                                                             const uint64_t file_size) = 0;

    // 上传分片，返回是否合并完成（true 表示已合并并生成文件）
    virtual Result<bool> UploadPart(const std::string &upload_id, const uint32_t split_index, const uint32_t split_num,
                                    const std::string &temp_file_path) = 0;

    // 单文件上传
    virtual Result<model::MediaFile> UploadFile(const uint64_t user_id, const std::string &file_name,
