    ...
    for (auto &s : httpServers) {
        ...
        dispatch->addServlet(
            ...
            [this](IM::http::HttpRequest::ptr req, IM::http::HttpResponse::ptr res, IM::http::HttpSession::ptr) {
                ...
                auto svc_ret = m_message_service->DeleteMessages(uid_ret.data, talk_mode, to_from_id, msg_ids);
                if (!svc_ret.ok) {
        ...
        dispatch->addServlet(
            ...
            [this](IM::http::HttpRequest::ptr req, IM::http::HttpResponse::ptr res, IM::http::HttpSession::ptr) {
                ...
                auto svc_ret =
                    m_message_service->LoadHistoryRecords(uid_ret.data, talk_mode, to_from_id, msg_type, cursor, limit);
                if (!svc_ret.ok) {
                    res->setStatus(ToHttpStatus(svc_ret.code));
                    res->setBody(Error(svc_ret.code, svc_ret.err));
                    return 0;
        ...
        dispatch->addServlet(
            ...
            [this](IM::http::HttpRequest::ptr req, IM::http::HttpResponse::ptr res, IM::http::HttpSession::ptr) {
                ...
                auto svc_ret = m_message_service->LoadRecords(uid_ret.data, talk_mode, to_from_id, cursor, limit);
                if (!svc_ret.ok) {
                    res->setStatus(ToHttpStatus(svc_ret.code));
                    res->setBody(Error(svc_ret.code, svc_ret.err));
                    return 0;
        ...
        dispatch->addServlet(
            ...
            [this](IM::http::HttpRequest::ptr req, IM::http::HttpResponse::ptr res, IM::http::HttpSession::ptr) {
                ...
                auto svc_ret = m_message_service->RevokeMessage(uid_ret.data, talk_mode, to_from_id, msg_id);
                if (!svc_ret.ok) {
                    res->setStatus(ToHttpStatus(svc_ret.code));
                    res->setBody(Error(svc_ret.code, svc_ret.err));
                    return 0;
        ...
        dispatch->addServlet("/api/v1/message/send", [this](IM::http::HttpRequest::ptr req,
            ...
            if (msg_type == static_cast<uint16_t>(IM::common::MessageType::Forward) &&
                ...
                if (payload.isMember("msg_ids") && payload["msg_ids"].isArray()) {
                    if (!IM::common::parseMsgIdsFromJson(payload["msg_ids"], forward_msg_ids, true)) {
                        res->setStatus(ToHttpStatus(400));
                        res->setBody(Error(400, "msg_ids 格式错误"));
                        return 0;
                    }
                ...
                auto send_to_target = [&](uint8_t target_mode, uint64_t target_id, const Json::Value &forward_payload) {
                    Json::StreamWriterBuilder wb;
                    std::string fextra = Json::writeString(wb, forward_payload);
                    // 不传 msg_id 保证每个目标生成独立 ID（也可以根据需求由前端传入）
                    auto r =
                        m_message_service->SendMessage(uid_ret.data, target_mode, target_id, msg_type, std::string(),
                                                       fextra, quote_id, std::string(), std::vector<uint64_t>());
                    Json::Value item;
                    if (!r.ok) {
                        item["ok"] = false;
                        item["err"] = r.err;
            ...
            auto svc_ret = m_message_service->SendMessage(uid_ret.data, talk_mode, to_from_id, msg_type, content_text,
                                                          extra, quote_id, msg_id, mentioned_user_ids);
            if (!svc_ret.ok) {
                res->setStatus(ToHttpStatus(svc_ret.code));
                res->setBody(Error(svc_ret.code, svc_ret.err));
                return 0;
        ...
        dispatch->addServlet("/api/v1/message/status", [this](IM::http::HttpRequest::ptr req,
                                                              IM::http::HttpResponse::ptr res,
                                                              IM::http::HttpSession::ptr) {
            res->setHeader("Content-Type", "application/json");
            Json::Value body;
            ...
            auto svc_ret = m_message_service->UpdateMessageStatus(uid_ret.data, talk_mode, to_from_id, msg_id, status);
            if (!svc_ret.ok) {
                res->setStatus(ToHttpStatus(svc_ret.code));
                res->setBody(Error(svc_ret.code, svc_ret.err));
                return 0;
