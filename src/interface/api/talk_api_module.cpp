    ...
    for (auto &s : httpServers) {
        ...
        dispatch->addServlet(
            ...
            [this](IM::http::HttpRequest::ptr req, IM::http::HttpResponse::ptr res, IM::http::HttpSession::ptr) {
                ...
                // 清除未读消息数
                auto result = m_talk_service->clearSessionUnreadNum(uid_result.data, to_from_id, talk_mode);
                if (!result.ok) {
        ...
        // 清空会话聊天记录(单向)
        dispatch->addServlet(
            "/api/v1/talk/session-clear-records",
            [this](IM::http::HttpRequest::ptr req, IM::http::HttpResponse::ptr res, IM::http::HttpSession::ptr) {
                res->setHeader("Content-Type", "application/json");
                ...
                // 清空聊天记录
                auto result = m_message_service->ClearTalkRecords(uid_result.data, talk_mode, to_from_id);
                if (!result.ok) {
                    res->setStatus(ToHttpStatus(result.code));
                    res->setBody(Error(result.code, result.err));
                    return 0;
        ...
        dispatch->addServlet(
            ...
            [this](IM::http::HttpRequest::ptr req, IM::http::HttpResponse::ptr res, IM::http::HttpSession::ptr) {
                ...
                // 设置是否免打扰
                auto result = m_talk_service->setSessionDisturb(uid_result.data, to_from_id, talk_mode, action);
                if (!result.ok) {
                    res->setStatus(ToHttpStatus(result.code));
                    res->setBody(Error(result.code, result.err));
                    return 0;
        ...
        dispatch->addServlet(
            ...
            [this](IM::http::HttpRequest::ptr req, IM::http::HttpResponse::ptr res, IM::http::HttpSession::ptr) {
                ...
                // 设置是否置顶
                auto result = m_talk_service->setSessionTop(uid_result.data, to_from_id, talk_mode, action);
                if (!result.ok) {
                    res->setStatus(ToHttpStatus(result.code));
                    res->setBody(Error(result.code, result.err));
                    return 0;
