...
bool MessageModule::handleRockRequest(IM::RockRequest::ptr request, IM::RockResponse::ptr response,
    ...
    if (cmd == kCmdSendMessage) {
        ...
        auto r = m_message_service->SendMessage(current_user_id, talk_mode, to_from_id, msg_type, content_text, extra,
                                                quote_msg_id, msg_id, mentioned_user_ids);
        if (!r.ok) {
    ...
    if (cmd == kCmdLoadHistoryRecords) {
        ...
        const uint64_t cursor = IM::JsonUtil::GetUint64(body, "cursor");
        const uint32_t limit = IM::JsonUtil::GetUint32(body, "limit");
        auto r = m_message_service->LoadHistoryRecords(current_user_id, talk_mode, to_from_id, msg_type, cursor, limit);
        if (!r.ok) {
            set_error(r.code == 0 ? 500 : r.code, r.err);
            return true;
        }
    ...
    if (cmd == kCmdUpdateMessageStatus) {
        ...
        auto r = m_message_service->UpdateMessageStatus(current_user_id, talk_mode, to_from_id, msg_id, status);
        if (!r.ok) {
            set_error(r.code == 0 ? 500 : r.code, r.err);
            return true;
        }
