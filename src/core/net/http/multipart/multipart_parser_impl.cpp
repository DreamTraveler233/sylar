...
class SimpleMultipartParser : public MultipartParser {
   public:
    bool Parse(const std::string &body, const std::string &content_type, const std::string &temp_dir,
               std::vector<Part> &parts, std::string *err = nullptr) override {
        std::string boundary = GetBoundary(content_type);
        // IM_LOG_INFO(IM_LOG_NAME("root")) << "Parse start. Content-Type: " << content_type << ", Boundary: '" <<
        // boundary << "'";
        ...
        if (pos == std::string::npos) {
            ...
            if (body.size() > 0) {
                IM_LOG_DEBUG(IM_LOG_NAME("root"))
                    << "Body start: " << body.substr(0, std::min<size_t>(100, body.size()));
            }
            return false;
        }
        ...
        while (true) {
            ...
            if (start_part >= end_part) {
                IM_LOG_WARN(IM_LOG_NAME("root"))
                    << "Empty part or malformed? start=" << start_part << ", end=" << end_part;
                pos = next_pos;
                continue;
            }
            ...
            // IM_LOG_INFO(IM_LOG_NAME("root")) << "Part found. Header size: " << header_str.size() << ", Data size: "
            // << data_str.size();
