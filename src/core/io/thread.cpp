...
Thread::Thread(std::function<void()> cb, const std::string &name) : m_id(-1), m_thread(-1), m_cb(cb), m_name(name) {
    /**
     * 这里将 this 传入线程函数的原因：
     ...
     *      2、void*
     * (*)(void*)表示一个指向“返回值为void*”“函数参数为void*”的函数的指针，即函数指针，void*表示可以指向任意类型的数据
     */
