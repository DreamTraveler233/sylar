...
class Murmur3Hash<std::vector<T>, seed> {
   public:
    uint32_t operator()(const std::vector<T> &data) { return murmur3_hash(&data[0], data.size() * sizeof(T), seed); }
};
...
class Murmur3Hash64<std::string, seed, seed2> {
   public:
    uint64_t operator()(const std::string &data) { return murmur3_hash64(data.data(), data.size(), seed, seed2); }
};
