...
class ModuleCloser {
   public:
    ...
    void operator()(Module *module) {
        ...
        if (rt) {
            IM_LOG_ERROR(g_logger) << "dlclose handle fail handle=" << m_handle << " name=" << name
                                   << " version=" << version << " path=" << path << " error=" << dlerror();
        } else {
            IM_LOG_INFO(g_logger) << "destory module=" << name << " version=" << version << " path=" << path
...
Module::ptr Library::GetModule(const std::string &path) {
    ...
    if (!create) {
        IM_LOG_ERROR(g_logger) << "cannot load symbol CreateModule in " << path << " error=" << dlerror();
        dlclose(handle);
        return nullptr;
    }
    ...
    if (!destory) {
        IM_LOG_ERROR(g_logger) << "cannot load symbol DestoryModule in " << path << " error=" << dlerror();
        dlclose(handle);
        return nullptr;
    }
    ...
    IM_LOG_INFO(g_logger) << "load module name=" << module->getName() << " version=" << module->getVersion()
