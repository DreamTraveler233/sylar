...
namespace IM::app
{
                                                                    ...
                                                                    uint64_t cursor, uint32_t limit)
    {
        ...
        for (auto &m : msgs)
        {
            if (msg_type != 0 && m.msg_type != msg_type)
                continue;
            dto::MessageRecord rec;
            std::string rerr;
            buildRecord(m, rec, &rerr);
        ...
        const std::vector<std::string> &msg_ids)
    {
        ...
        for (auto &mid : msg_ids)
        {
            model::Message m;
            std::string merr;
            if (!m_message_repo->GetById(mid, m, &merr))
                continue; // 忽略不存在
            dto::MessageRecord rec;
            std::string rerr;
            buildRecord(m, rec, &rerr);
        ...
        const std::vector<uint64_t> &mentioned_user_ids)
    {
        ...
        if (talk_mode == 1)
        {
            ...
            {
                trans->rollback();
                IM_LOG_ERROR(g_logger) << "SendMessage GetContactDetail(receiver_view) failed, err="
                                       << rcv.err;
        ...
        if (m.msg_type == static_cast<uint16_t>(IM::common::MessageType::Forward) && !m.extra.empty())
        {
            ...
            if (Json::parseFromStream(rb, in, &payload, &errs))
            {
                ...
                if (!src_ids.empty())
                {
                    if (src_ids.size() > kMaxPreview)
                        src_ids.resize(kMaxPreview);
        ...
        if (mtype == IM::common::MessageType::Text)
        {
            ...
            if (last_msg_digest.size() > 255)
                last_msg_digest = last_msg_digest.substr(0, 255);
        }
        else
        {
            auto it = IM::common::kMessagePreviewMap.find(mtype);
        ...
        if (talk_mode == 1)
        {
            ...
            try
            {
                ...
                if (deliver_to_receiver)
                {
                    ...
                    if (!cd_receiver.nickname.empty())
                        session_receiver.name = cd_receiver.nickname;
                    if (!cd_receiver.avatar.empty())
                        session_receiver.avatar = cd_receiver.avatar;
