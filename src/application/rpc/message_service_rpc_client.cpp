...
namespace IM::app::rpc
{
    ...
    MessageServiceRpcClient::MessageServiceRpcClient()
        : m_rpc_addr(IM::Config::Lookup("message.rpc_addr", std::string(""),
                                        "svc-message rpc address ip:port")) {}

    uint64_t MessageServiceRpcClient::resolveTalkId(const uint8_t /*talk_mode*/,
                                                    const uint64_t /*to_from_id*/)
    {
        return 0;
    }

    bool MessageServiceRpcClient::buildRecord(const IM::model::Message & /*msg*/,
                                              IM::dto::MessageRecord & /*out*/,
                                              std::string *err)
    {
        if (err)
        {
    ...
    bool MessageServiceRpcClient::GetTalkId(const uint64_t /*current_user_id*/,
                                            const uint8_t /*talk_mode*/,
                                            const uint64_t /*to_from_id*/, uint64_t &talk_id,
                                            std::string &err)
    {
        talk_id = 0;
    ...
    Result<void> MessageServiceRpcClient::callVoid(uint32_t cmd,
                                                   const std::function<void(Json::Value &)> &fill)
    {
        Result<void> result;
        ...
        const std::vector<uint64_t> &mentioned_user_ids)
    {
        ...
        if (!mentioned_user_ids.empty())
        {
            ...
            for (auto id : mentioned_user_ids)
                arr.append((Json::UInt64)id);
    ...
    Result<IM::dto::MessagePage> MessageServiceRpcClient::LoadHistoryRecords(
        const uint64_t current_user_id, const uint8_t talk_mode, const uint64_t to_from_id,
        const uint16_t msg_type, uint64_t cursor, uint32_t limit)
    {
        Result<IM::dto::MessagePage> result;
    ...
    Result<void> MessageServiceRpcClient::DeleteMessages(const uint64_t current_user_id,
                                                         const uint8_t talk_mode,
                                                         const uint64_t to_from_id,
                                                         const std::vector<std::string> &msg_ids)
    {
    ...
    Result<void> MessageServiceRpcClient::ClearTalkRecords(const uint64_t current_user_id,
                                                           const uint8_t talk_mode,
                                                           const uint64_t to_from_id)
    {
    ...
    Result<void> MessageServiceRpcClient::RevokeMessage(const uint64_t current_user_id,
                                                        const uint8_t talk_mode,
                                                        const uint64_t to_from_id,
    ...
    Result<void> MessageServiceRpcClient::UpdateMessageStatus(const uint64_t current_user_id,
                                                              const uint8_t talk_mode,
                                                              const uint64_t to_from_id,
                                                              const std::string &msg_id,
                                                              uint8_t status)
