/**
 * @file user_service_rpc_client.hpp
 * @brief RPC客户端实现
...
class UserServiceRpcClient : public IM::domain::service::IUserService {
   public:
    ...
    Result<IM::model::User> GetUserByMobile(const std::string &mobile, const std::string &channel) override;

    Result<IM::model::User> GetUserByEmail(const std::string &email, const std::string &channel) override;
