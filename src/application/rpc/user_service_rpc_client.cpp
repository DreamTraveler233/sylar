...
namespace IM::app::rpc
{
    ...
    UserServiceRpcClient::UserServiceRpcClient()
        : m_rpc_addr(IM::Config::Lookup("user.rpc_addr", std::string(""),
                                        "svc-user rpc address ip:port")) {}

    IM::RockResult::ptr UserServiceRpcClient::rockJsonRequest(const std::string &ip_port,
                                                              uint32_t cmd,
                                                      ...
                                                      const std::string &new_password)
    {
        ...
        const auto addr = resolveSvcUserAddr();
        return FromRockVoid(rockJsonRequest(addr, kCmdUpdatePassword, req, kTimeoutMs),
                            "svc-user unavailable");
    }
                                                      ...
                                                      const uint32_t gender, const std::string &birthday)
    {
        ...
        const auto addr = resolveSvcUserAddr();
        return FromRockVoid(rockJsonRequest(addr, kCmdUpdateUserInfo, req, kTimeoutMs),
                            "svc-user unavailable");
    }
                                                    ...
                                                    const std::string &sms_code)
    {
        ...
        const auto addr = resolveSvcUserAddr();
        return FromRockVoid(rockJsonRequest(addr, kCmdUpdateMobile, req, kTimeoutMs),
                            "svc-user unavailable");
    }
                                                   ...
                                                   const std::string &email_code)
    {
        ...
        const auto addr = resolveSvcUserAddr();
        return FromRockVoid(rockJsonRequest(addr, kCmdUpdateEmail, req, kTimeoutMs),
                            "svc-user unavailable");
    }

    Result<IM::model::User> UserServiceRpcClient::GetUserByMobile(const std::string &mobile,
    ...
    Result<IM::model::User> UserServiceRpcClient::GetUserByEmail(const std::string &email,
                                                                 const std::string &channel)
    {
        Result<IM::model::User> result;
        Json::Value req(Json::objectValue);
    ...
    Result<void> UserServiceRpcClient::SaveConfigInfo(const uint64_t user_id,
                                                      const std::string &theme_mode,
                                                      ...
                                                      const std::string &keyboard_event_notify)
    {
        ...
        const auto addr = resolveSvcUserAddr();
        return FromRockVoid(rockJsonRequest(addr, kCmdSaveConfigInfo, req, kTimeoutMs),
                            "svc-user unavailable");
    }
    ...
    Result<IM::model::User> UserServiceRpcClient::Authenticate(const std::string &mobile,
    ...
    Result<void> UserServiceRpcClient::LogLogin(const Result<IM::model::User> &user_result,
                                                const std::string &platform,
    ...
    Result<void> UserServiceRpcClient::GoOnline(const uint64_t id)
    {
        ...
        const auto addr = resolveSvcUserAddr();
        return FromRockVoid(rockJsonRequest(addr, kCmdGoOnline, req, kTimeoutMs),
                            "svc-user unavailable");
    }

    Result<IM::model::User> UserServiceRpcClient::Register(const std::string &nickname,
    ...
    Result<IM::model::User> UserServiceRpcClient::Forget(const std::string &mobile,
                                                         const std::string &new_password,
                                                         const std::string &sms_code)
    {
        Result<IM::model::User> result;
